...
const ctx = {
    ...
    stroke: () => {}, fillRect: () => {}, arc: () => {}, fill: () => {}, fillText: () => {}
};
...
for (let i = 0; i < 500; i++) {
    nodes.push({
        ...
        mass: Math.floor(Math.random() * 8) + 10, charge: Math.random() > 0.5 ? 1.0 : -1.0,
        name: `Being:${i}`
    });
}
...
console.log(`Optimized time: ${(end - start).toFixed(2)} ms for 50 frames with 500 nodes`);
